...
//    private RRSet generateRRSet2(Node node1) {
...
//        while (!queue.isEmpty()) {
//            nextNode = queue.poll();
//            nextNode.getTrip().getBeginStation().addRRSet(rrSet);
//            nextNode.getTrip().getEndStation().addRRSet(rrSet);
//            rrSet.addNode();
...
//    }
    ...
    //add RRSet to station while generating
    private RRSet generateRRSet2(Node node) {
        RRSet rrSet = new RRSet(RRSetSet.size());
        Queue<Node> queue = new ArrayDeque<>();
        Set<Node> visitedNodeSet = new HashSet<>();
        queue.add(node);
...
/*    private double getLambda() {
    ...
    private double getLambda() {
        ...
        double com = Combination.getC3((int) nodeNum, checkPointNum);
        double comLog;
        ...
        double b = (Setting.L * Math.log(nodeNum) + comLog + Math.log(2.0));
        return a * b;
    }
